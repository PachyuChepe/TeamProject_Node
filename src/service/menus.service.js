...
class MenusService {
  ...
  // 메뉴 저장
  createMenu = async (ownerId, name, description, price, imageUrl) => {
    // 조회 : 매장 번호
    const storeId = await this.menusRepository.getStoreId(ownerId);

    // 저장 : 메뉴 정보
    const menu = await this.menusRepository.createMenu(
      storeId,
      name,
      description,
      price,
  ...
  // 메뉴 수정
  updateMenu = async (ownerId, id, name, description, price, imageUrl) => {
    // 조회 : 회원 번호
    const storeId = await this.menusRepository.getStoreId(ownerId);
    // 조회 : 메뉴 정보
    const existsMenu = await this.menusRepository.getMenu(id);
    ...
    // 수정 : 메뉴 정보
    const menu = await this.menusRepository.updateMenu(
      id,
      name,
      description,
      price,
