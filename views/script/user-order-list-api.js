  ...
  axios
    ...
    .then((response) => {
      ...
      response.data.data.forEach((e, idx) => {
        ...
        const btnHref = e.review.length > 0 ?
          `user-review-edit.html?storeId=${e.menu.store.id}&id=${review}` : `user-review-create.html?storeId=${e.menu.store.id}&orderId=${e.id}`;
        const comment = e.review.length > 0 ? e.review[0].comment : "리뷰 작성 전";
